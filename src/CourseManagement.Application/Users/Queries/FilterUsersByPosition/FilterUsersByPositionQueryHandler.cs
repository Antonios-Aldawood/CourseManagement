...
{
    public class FilterUsersByPositionQueryHandler(
        IUsersRepository usersRepository
        ) : IRequestHandler<FilterUsersByPositionQuery, ErrorOr<List<UserDto>>>
    {
        private readonly IUsersRepository _usersRepository = usersRepository;

        public async Task<ErrorOr<List<UserDto>>> Handle(FilterUsersByPositionQuery query, CancellationToken cancellationToken)
        {
            try
            {
                var filteredUsers = await _usersRepository.FilterByPositionAsync(query.position);
