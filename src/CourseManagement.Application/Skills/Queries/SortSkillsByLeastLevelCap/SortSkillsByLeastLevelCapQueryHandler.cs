...
{
    public class SortSkillsByLeastLevelCapQueryHandler(
        ISkillsRepository skillsRepository
        ) : IRequestHandler<SortSkillsByLeastLevelCapQuery, ErrorOr<List<SkillSoleDto>>>
    {
        private readonly ISkillsRepository _skillsRepository = skillsRepository;

        public async Task<ErrorOr<List<SkillSoleDto>>> Handle(SortSkillsByLeastLevelCapQuery query, CancellationToken cancellationToken)
        {
            try
            {
                var foundSkills = await _skillsRepository.SortSkillsByLeastLevelCapAsync();
