...
{
    public class PaginatedGetAllJobsQueryHandler(
        IJobsRepository jobsRepository
        ) : IRequestHandler<PaginatedGetAllJobsQuery, ErrorOr<PagedResult<JobDto>>>
    {
        private readonly IJobsRepository _jobsRepository = jobsRepository;

        public async Task<ErrorOr<PagedResult<JobDto>>> Handle(PaginatedGetAllJobsQuery query, CancellationToken cancellationToken)
        {
            try
            {
                var (jobs, totalCount) = await _jobsRepository
                    .GetAllPaginatedJobsAsync(query.pageNumber, query.pageSize);
