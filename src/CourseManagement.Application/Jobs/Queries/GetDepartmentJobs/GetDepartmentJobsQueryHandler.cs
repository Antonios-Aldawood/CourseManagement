...
{
    public class GetDepartmentJobsQueryHandler(
        IJobsRepository jobsRepository,
        IDepartmentsRepository departmentsRepository
        ) : IRequestHandler<GetDepartmentJobsQuery, ErrorOr<List<JobDto>>>
    {
        ...
        public async Task<ErrorOr<List<JobDto>>> Handle(GetDepartmentJobsQuery query, CancellationToken cancellationToken)
        {
            try
            {
                ...
                List<JobDto> jobs = await _jobsRepository.GetDepartmentJobsAsync(department.Id);
                if (jobs is null ||
                    jobs.Count == 0)
                {
