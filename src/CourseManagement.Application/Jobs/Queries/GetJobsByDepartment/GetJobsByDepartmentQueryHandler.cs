...
{
    public class GetJobsByDepartmentQueryHandler(
        IJobsRepository jobsRepository
        ) : IRequestHandler<GetJobsByDepartmentQuery, ErrorOr<List<JobDto>>>
    {
        private readonly IJobsRepository _jobsRepository = jobsRepository;

        public async Task<ErrorOr<List<JobDto>>> Handle(GetJobsByDepartmentQuery query, CancellationToken cancellationToken)
        {
            try
            {
                var foundJobs = await _jobsRepository.GetJobsByDepartmentAsync(query.name);
