...
{
    public class GetDepartmentsJobsQueryHandler(
        IJobsRepository jobsRepository,
        IDepartmentsRepository departmentsRepository
        ) : IRequestHandler<GetDepartmentsJobsQuery, ErrorOr<List<JobDto>>>
    {
        ...
        public async Task<ErrorOr<List<JobDto>>> Handle(GetDepartmentsJobsQuery query, CancellationToken cancellationToken)
        {
            try
            {
                ...
                List<JobDto> jobs = await _jobsRepository.GetDepartmentsJobsAsync(departmentIds);
