...
{
    public class FilterJobsBySalaryQueryHandler(
        IJobsRepository jobsRepository
        ) : IRequestHandler<FilterJobsBySalaryQuery, ErrorOr<List<JobDto>>>
    {
        private readonly IJobsRepository _jobsRepository = jobsRepository;

        public async Task<ErrorOr<List<JobDto>>> Handle(FilterJobsBySalaryQuery query, CancellationToken cancellationToken)
        {
            try
            {
                var foundJobs = await _jobsRepository.FilterJobsBySalaryAsync(query.salary);
